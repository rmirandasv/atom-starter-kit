...
export default function ShowTeamPage({ team, members }: { team: Team; members: User[] }) {
  ...
  return (
    ...
    >
      <Heading title={team.name} description="Manage your team settings and members." size="small" />
      <div className="flex flex-col rounded-md border border-border p-4">
        <div className="flex flex-row items-center justify-between">
          <div className="flex flex-col">
            <span className="text-sm font-medium">{team.name}</span>
            {team.personal_team === true && team.user_id === auth.user.id && <span className="text-sm text-muted-foreground">Personal Team</span>}
            {!team.personal_team && <span className="text-sm text-muted-foreground">{members.find((m) => m.id === auth.user.id)?.pivot?.role}</span>}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {(team.user_id === auth.user.id || team.members.find((member) => member.id === auth.user.id)?.pivot?.role === "admin") && (
            ...
            <Fragment key={`member-${member.id}`}>
              <li className="flex flex-row items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">{member.pivot?.role}</span>
                </div>
                {team.user_id === auth.user.id && member.id !== auth.user.id && (
                  <Button variant="ghost" size="sm" disabled={loading} onClick={() => handleRemoveMember(member.id)}>
                    {loading ? "Removing..." : "Remove"}
                  </Button>
                )}
              </li>
              {index < members.length - 1 && <Separator orientation="horizontal" className="" />}
            </Fragment>
          ))}
        </ul>
          ...
          <Fragment>
            ...
            <ul className="mt-4 flex flex-col gap-2">
              {team.invitations.map((invite) => (
                <li key={`invite-${invite.id}`} className="flex flex-row items-center justify-between">
      ...
      {team.user_id === auth.user.id && team.personal_team == false && (
        <div className="mt-4 rounded-md border border-red-200 bg-red-50 p-4">
          ...
          <p className="mt-1 text-sm text-red-700">
            Deleting a team will remove all of its members and their access to the team. This action cannot be undone.
          </p>
          <Button variant="destructive" className="mt-2" onClick={handleDeleteTeam} disabled={loading}>
      ...
      {team.user_id !== auth.user.id && (
        <div className="mt-4 rounded-md border border-red-200 bg-red-50 p-4">
          ...
          <p className="mt-1 text-sm text-red-700">Leaving a team will remove you from the team and its members. This action cannot be undone.</p>
          <Button variant="destructive" className="mt-2" onClick={handleLeaveTeam} disabled={loading}>
